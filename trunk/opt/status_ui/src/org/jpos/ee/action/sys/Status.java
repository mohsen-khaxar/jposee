...
public class Status extends ActionSupport {
    public void execute (JPublishContext context, Configuration cfg) {
        ...
        try {
            ...
            Query query = null;
            if (tag!=null) {
                Criteria c = db.session().createCriteria(StatusTag.class);
                if (!Validator.isQuery(tag)) {
                    throw new BLException ("Invalid characters in parameter tag.");
                }
                c.add(Property.forName("tag").in(tag.split(" ")));
                List l = new ArrayList();
                Iterator it = c.list().iterator();
                while (it.hasNext()) {
                  l.add(((StatusTag)it.next()).getStatus());
                }
                context.put ("status", l.iterator());
            } else {
                qs.append("from status in class org.jpos.ee.status.Status");
                qs.append (" order by groupName, name, id");
                query = db.session().createQuery (qs.toString());
                context.put ("status", query.iterate());
            }
